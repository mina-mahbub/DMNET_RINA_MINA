...
class Trainer(object):
    ...
    def train(self):
        ...
        if self.framework == 'torch':
            ...
            mycolumns1= ['Epoch', 'Train_Loss']
            df1=pd.DataFrame(columns = mycolumns1)

            mycolumns= ['Epoch', 'Val_Loss']
                ...
                for batch_idx, batch in enumerate(self.train_datagen):
                    ...
                    if torch.cuda.is_available():
                        ...
                        if self.config['data_specs'].get('additional_inputs',None) is not None:
                            data = []
                            for i in ['image'] + self.config['data_specs']['additional_inputs']:
            ...
            # df.to_csv(os.path.join(dir_path, "Val_Loss"),index=True)
