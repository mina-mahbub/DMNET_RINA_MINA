...
def get_centermask(da_name,mask):
    if da_name not in ['BF-C2DL-HSC' ,'BF-C2DL-MuSC','Fluo-C2DL-MSC', 'Fluo-N2DH-GOWT1', 'Fluo-N2DL-HeLa', 'Fluo-C3DL-MDA231', 'Fluo-N3DH-CHO','Fluo-C3DH-H157']:
...
thre = {"Fluo-N2DL-HeLa": [5, 3], "DIC-C2DH-HeLa": [20, 3], "Fluo-C2DL-MSC": [5, 3], "PhC-C2DH-U373": [10, 3],"Cell-Data": [20, 3], "Cell-Data-P1": [20, 3], "Cell-Data-P2":[20,3],
        "Cell-Data-M": [20, 3],"PhC-C2DL-PSC": [3, 3], 'Fluo-N2DH-GOWT1': [10, 3], 'BF-C2DL-HSC': [5, 5], 'BF-C2DL-MuSC': [7, 3],
        'Fluo-C3DH-A549':[5,3],
...
def rand_crop(image, label,dist,cropsize):
    h, w = image.shape[:-1] # h=472, w=1064

    asv=int(cropsize/2.)  # asv=128
    x = random.randint(asv, w - asv) # 128, 936
    y = random.randint(asv, h - asv) # 128, 344
    image = image[y-asv:y + asv, x-asv:x + asv] #0:256, 0:256, 216:472
...
class TorchDataset(Dataset):
    ...
    def __init__(self, branch,df, batch_size,sizecrop,stage="train",num_classes=1):
        ...
        self.stage=stage
        self.aug_el=iaa.Sequential([iaa.Affine(rotate=(-45, 45)),
                                    iaa.Flipud(0.5),
                                    iaa.Fliplr(0.5),
                                    iaa.PiecewiseAffine(scale=(0.01, 0.05)),
                                    iaa.Sometimes(0.5,iaa.ElasticTransformation(alpha=(25.0,75.0),sigma=10.0)),
                                    iaa.Sometimes(0.3,iaa.blur.GaussianBlur(sigma=[1.0,2.75])),
    ...
    def __getitem__(self, idx):
        ...
        # print(mask_new.shape, image.shape)
        ...
        if sample['image'].shape != (3, 256, 256):
            print(f'Image shape error at Idx {idx} (path: {img_path}): shape = {sample["image"].shape}')
        if sample['mask'].shape != (1, 256, 256):
            print(f'Mask shape error at idx {idx} (path: {img_path}): shape = {sample["mask"].shape}')
        if sample['dist_mask'].shape != (1, 256, 256):
            print(f'dist_mask shape error at idx {idx} (path: {img_path}): shape = {sample["dist_mask"].shape}')
        return sample
