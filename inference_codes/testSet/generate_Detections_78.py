...
area_thresh_all = {"BF-C2DL-MuSC": [0, 100],
                   ...
                   "Cell-Data-P2": [15500, 0.90, 0.50],  # Area threshold, marker threshold, mask threshold # [1200, 0.7, 0.1]  # Previous results were obtained at # [6600, 0.99, 0.5]
...
def get_detection(input_imgpath, save_rina_seg_path, da, gttype, model_choose="final", vis=0, all=0):
    ...
    for imgp in tqdm(imglist):
        ...
        # mask=contrast_strech_norm(maskb)                                  ####   Avoiding  adapthist
        # plt.imshow(np.concatenate([maskb,mask],1))
        # plt.show()
        # center_pred = regular_norm(center_pred)                             ### Avoiding  adapthist
        # plt.imshow(maskb)
        ...
        elif da in ['Cell-Data', 'Cell-Data-P1', 'Cell-Data-P2','Cell-Data-M']:
            mask = regular_norm(maskb)
            center_pred = center_inferer([imgp, da])
            center_pred = center_inferer([imgp, da])
            center_pred = regular_norm(center_pred)
            center_marker_det = postprocess_mask_and_markers(mask, center_pred, area_thresh=area_thresh_all[da][0],
                                                             marker_thresh=area_thresh_all[da][1], mask_thresh=area_thresh_all[da][2])  #0.99, 0.7
            # mask = regular_norm(maskb)

            # center_pred = center_inferer([imgp, da])
            # # center_pred = contrast_strech_norm(center_pred)

            # center_marker_det = postprocess_mask_and_watermarkers(mask, center_pred, area_thresh=area_thresh_all[da][1])
