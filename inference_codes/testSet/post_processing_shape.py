...
def remove_small_components(tensor, area_thresh=200, mask_thresh=.5):
    out = (tensor > mask_thresh).astype('uint8') * 255
    ...
    out = (cc < mask_thresh).astype('uint8') * 255
    nc, cc, stats, _ = cv2.connectedComponentsWithStats(out)
    for i, stat in enumerate(stats):
        area = stat[-1]
        if area < area_thresh:
    ...
    cc = (cc < mask_thresh).astype('float32')
    return cc
...
def postprocess_mask_and_markers(mask, markers_ori, area_thresh=50, marker_thresh=.5, mask_thresh=.5, closing=False):
    # Label markers if not labeled

    # print(markers_ori.shape)
    # plt.imshow(markers_ori)
    # plt.savefig('/home/MinaHossain/DMNet_Rina/test_1.png')
    markers = measure.label(markers_ori.copy()>marker_thresh)
    # plt.imshow(markers)
    # plt.savefig('/home/MinaHossain/DMNet_Rina/test_2.png')
    # Remove small components
    # plt.imshow(mask)
    # plt.savefig('/home/MinaHossain/DMNet_Rina/test_3.png')
    mask = remove_small_components(mask, area_thresh=area_thresh, mask_thresh=mask_thresh)
    # plt.imshow(mask)
    # plt.savefig('/home/MinaHossain/DMNet_Rina/test_4.png')
    if closing:
        mask=1.0*mask>0.0
    ...
    #plt.show()
    # plt.imshow(distance)
    # plt.savefig('/home/MinaHossain/DMNet_Rina/test_5.png')
    # plt.imshow(-distance)
    # plt.savefig('/home/MinaHossain/DMNet_Rina/test_6.png')
    ...
    # plt.imshow(labels)
    # plt.savefig('/home/MinaHossain/DMNet_Rina/test_7.png')
    ...
    for i in range(1, labels.max() + 1):
        ...
        if np.sum(labels == i) < area_thresh:
            labels[labels == i] = 0
    # plt.imshow(labels)
    # plt.savefig('/home/MinaHossain/DMNet_Rina/test_8.png')
    return labels
...
def postprocess_mask_and_markers_3d(mask, markers, area_thresh=50):
    ...
    # Calculate distance transform for watershed
    distance = ndi.morphology.distance_transform_edt(mask > 0, return_distances=True, return_indices=False)
